...
### "Something went wrong. Please try again." on Sign-In

This is the most common Teams authentication error. Check these in order:

**1. OAuth Connection Name Mismatch**

The name in Azure Bot Service must match the code:
- Azure Portal: Azure Bot → Settings → OAuth Connection Settings → Name
- Code: `appsettings.json` → `AgentApplication:UserAuthorization:Handlers:graph:Settings:AzureBotOAuthConnectionName`

Both must be `GraphConnection` (case-sensitive).

**2. Missing Valid Domains**
...
**3. OAuth Connection Not Configured**

1. Go to Azure Portal → Azure Bot → Settings → OAuth Connection Settings
2. Click **Add Setting** or verify existing
3. Test the connection with **Test Connection** button

### Sign-in Opens But Doesn't Return to Teams

**Cause**: Missing redirect URI in Azure AD app.
...
**Cause**: Azure AD app doesn't have the correct redirect URI.
...
### SSO Token Exchange Fails

**Cause**: SSO not properly configured.

**Solution**:
1. **Expose an API** in Azure AD app:
   - Set Application ID URI: `api://your-app.azurewebsites.net/{client-id}`
   - Add scope: `access_as_user`

2. **Pre-authorize Teams clients**:
   - `1fec8e78-bce4-4aaf-ab1b-5451cc387264` (Teams desktop/web)
   - `5e3ce6c0-2b1f-4285-8d4b-75ee78787346` (Teams mobile)

3. **Update manifest.json** with `webApplicationInfo`:
   ```json
   "webApplicationInfo": {
     "id": "your-client-id",
     "resource": "api://your-app.azurewebsites.net/your-client-id"
   }
   ```

### "AutoSignIn" Not Triggering Sign-In Card

**Cause**: `AutoSignIn` is set to `false` in configuration.
...
"AgentApplication": {
  "UserAuthorization": {
    "AutoSignIn": true,  // Must be true
    "DefaultHandlerName": "graph"
  }
}
```

### No Sign-In Card Appears at All

**Cause**: `UserAuthorization` section missing from configuration.
